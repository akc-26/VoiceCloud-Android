...
echo =============================================================
echo VoiceCloud Android PH13 R14 - Full Product/UI Acceptance
echo Parent package: VC-ANDROID-PH13-R13
echo Scope: physical-device-driven UI/UX correction with functional/API/RTC/payment/navigation authority protected
echo This gate restores the complete compile/test/lint/assembly/device acceptance sequence.
echo =============================================================
...
if errorlevel 1 (
  ...
  if errorlevel 1 (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -File scripts\VC-ANDROID-PH13-R14-SOURCE-DIAGNOSTIC.ps1
...
if "!VC_FAILURE_GROUPS!"=="0" (
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts\VC-ANDROID-PH13-R14-ISOLATED-BUILD.ps1
  ...
  if "!VC_ANDROID_RC!"=="2" (
    ...
    echo [GATE-FAIL] R14 full Android build/device gate failed.
    set /a VC_FAILURE_GROUPS+=1
  ) else (
    echo [GATE-PASS] R14 full Android build and physical-device instrumentation passed.
  )
) else (
  echo [GATE-SKIP] Full Android build/device acceptance skipped because a prerequisite/source gate failed.
)

echo [R14 INTEGRITY] Product/delivery integrity
where python >nul 2>nul
if errorlevel 1 (
  echo [GATE-FAIL] Python unavailable; R14 delivery integrity could not run.
  set /a VC_FAILURE_GROUPS+=1
) else (
  python scripts\ph13_r14_delivery_integrity.py
  if errorlevel 1 set /a VC_FAILURE_GROUPS+=1
)

echo =============================================================
if not "!VC_FAILURE_GROUPS!"=="0" (
  echo [FAIL] VC-ANDROID-PH13-R14 acceptance completed with !VC_FAILURE_GROUPS! genuine failing group^(s^).
  exit /b 1
)
if "!VC_DEVICE_PENDING!"=="1" (
  echo [PENDING] VC-ANDROID-PH13-R14 is NOT fully accepted yet.
...
echo [PASS] VC-ANDROID-PH13-R14 FULL ACCEPTANCE completed successfully.
