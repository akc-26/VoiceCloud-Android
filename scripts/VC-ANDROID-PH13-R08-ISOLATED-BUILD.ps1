...
$workspace = Join-Path $env:TEMP ("VoiceCloud-PH13-R08-$runId")
$gradleUserHome = Join-Path $env:TEMP ("VoiceCloud-PH13-R08-GRADLE-$runId")
...
try {
    ...
    if ($script:compileGatesClean) {
        ...
        Write-Host '[GATE-SKIP] Gates 4-6 skipped because one or more mandatory Kotlin compile variants failed; fix compilation first.'
    }

    if ($script:compileGatesClean -and $script:assemblyGateClean) {
        $script:total++
        ...
        Write-Host '[GATE-SKIP] Gate 7 skipped because the APK/androidTest assembly prerequisites were not produced successfully.'
    }
} catch {
