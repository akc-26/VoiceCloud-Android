...
MANIFEST = ROOT / 'contracts/VC-ANDROID-PH13-R07-SOURCE-MANIFEST.sha256'
...
if not MANIFEST.is_file():
    raise SystemExit('[FAIL] PH13-R07 source manifest missing')
...
for line in MANIFEST.read_text(encoding='utf-8').splitlines():
    ...
    p = ROOT / rel
    if not p.is_file():
        raise SystemExit(f'[FAIL] PH13-R07 delivery file missing: {rel}')
    actual = hashlib.sha256(p.read_bytes()).hexdigest()
    if actual != expected:
        raise SystemExit(f'[FAIL] PH13-R07 delivery file drifted: {rel}')
...
print(f'[PASS] PH13-R07 tracked delivery manifest verified: {len(tracked)} files')
if ignored:
    print(f'[PASS] Ignored {len(ignored)} known workstation-generated artifact(s); product-source integrity remains unchanged')
print('[PASS] PH13-R07 product/delivery integrity is clean')
