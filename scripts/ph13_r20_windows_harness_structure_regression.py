...
files=[ROOT/'scripts/VC-ANDROID-PH13-R20-ISOLATED-BUILD.ps1',ROOT/'scripts/VC-ANDROID-PH13-R20-SOURCE-DIAGNOSTIC.ps1']
fail=[];passes=0
def clean(s):
...
cmd=(ROOT/'scripts/VC-ANDROID-PH13-R20-ACCEPTANCE.cmd').read_text(encoding='utf-8')
good='EnableDelayedExpansion' in cmd and '!ERRORLEVEL!' in cmd and 'set "VC_ANDROID_RC=!ERRORLEVEL!"' in cmd
print('[PASS]' if good else '[FAIL]','R20 CMD delayed ERRORLEVEL capture'); passes+=int(good); fail += [] if good else ['cmd']
good='set "PYTHONUTF8=1"' in cmd and 'set "PYTHONIOENCODING=utf-8"' in cmd
print('[PASS]' if good else '[FAIL]','R20 CMD locale-independent Python mode'); passes+=int(good); fail += [] if good else ['python-utf8']
if fail: raise SystemExit(1)
print(f'[PASS] VC-ANDROID-PH13-R20 Windows harness structure: {passes}/{passes} PASS')
