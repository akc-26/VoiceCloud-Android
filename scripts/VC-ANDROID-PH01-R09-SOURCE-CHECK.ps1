...
P 'endpoint tests match R09 resolver contract' ($test.Contains('liveServerRootExpandsToVersionedApiBase') -and $test.Contains('fullApiBaseIsNotDuplicated') -and $test.Contains('socketUsesServerOriginAndWebUsesTrailingSlash'))
...
$accept=T 'scripts/VC-ANDROID-PH01-R09-ACCEPTANCE.cmd'; P 'acceptance checks live server without adb reverse' ($accept.Contains('VC-ANDROID-LIVE-CONNECTIVITY-CHECK.cmd') -and -not $accept.Contains('VC-ANDROID-LOCAL-DEVICE-BACKEND.cmd'))
...
P 'wrapper generation uses cached local distribution URI' (T 'scripts/BOOTSTRAP-GRADLE-WRAPPER.ps1').Contains('--gradle-distribution-url $localDistributionUri')
P 'wrapper pins Gradle 9.5.0 distribution SHA-256' (T 'gradle/wrapper/gradle-wrapper.properties').Contains('distributionSha256Sum=553c78f50dafcd54d65b9a444649057857469edf836431389695608536d6b746')
P 'live connectivity probe retries transient failures' (T 'scripts/VC-ANDROID-LIVE-CONNECTIVITY-CHECK.ps1').Contains('[int]$attempts = 3')
Write-Host "VC-ANDROID-PH01-R09 Windows source authority: $script:count/$script:count PASS"
