...
with tempfile.TemporaryDirectory(prefix='vc-r15-artifact-regression-') as td:
    tree = Path(td)/'tree'
    ...
    run(tree, 'ph13_r15_product_preservation_regression.py', 0)
    run(tree, 'ph13_r15_source_hygiene.py', 0)
    run(tree, 'ph13_r15_delivery_integrity.py', 0)
    print(f'[PASS] R15 workstation/generated-artifact simulation accepted: {len(KNOWN_FILES)} artifacts including exact Room schema JSON')

    drift = tree/'feature/auth/src/main/java/app/voicecloud/feature/auth/ui/UnexpectedR15Drift.kt'
    write(drift, 'package app.voicecloud.feature.auth.ui\ninternal object UnexpectedR15Drift\n')
    preservation = subprocess.run([sys.executable, str(tree/'scripts/ph13_r15_product_preservation_regression.py')], cwd=tree, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    delivery = subprocess.run([sys.executable, str(tree/'scripts/ph13_r15_delivery_integrity.py')], cwd=tree, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    if preservation.returncode == 0 or delivery.returncode == 0:
        print(preservation.stdout); print(delivery.stdout)
        raise SystemExit('[FAIL] R15 gates did not fail closed for unknown product/delivery drift')
    print('[PASS] R15 preservation and delivery integrity remain fail-closed for unknown product/source drift')
