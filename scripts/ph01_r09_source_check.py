...
ok('R09 endpoint unit tests match three-authority resolver', all(x in endpoint_test for x in ['liveServerRootExpandsToVersionedApiBase','fullApiBaseIsNotDuplicated','socketUsesServerOriginAndWebUsesTrailingSlash','releaseUsesConfiguredLiveAuthoritiesWithoutDeviceRewrite']))
...
accept=text('scripts/VC-ANDROID-PH01-R09-ACCEPTANCE.cmd'); ok('acceptance verifies live Raspberry Pi connectivity', 'VC-ANDROID-LIVE-CONNECTIVITY-CHECK.cmd' in accept and 'VC-ANDROID-LOCAL-DEVICE-BACKEND.cmd' not in accept)
...
print(f'VC-ANDROID-PH01-R09 source authority: {len(checks)}/{len(checks)} PASS')

# R09 wrapper/bootstrap hardening
ok("wrapper generation uses local cached distribution URI", "--gradle-distribution-url $localDistributionUri" in text("scripts/BOOTSTRAP-GRADLE-WRAPPER.ps1"))
ok("Gradle 9.5.0 distribution checksum pinned", "distributionSha256Sum=553c78f50dafcd54d65b9a444649057857469edf836431389695608536d6b746" in text("gradle/wrapper/gradle-wrapper.properties"))
ok("Pi connectivity retries transient failures", "[int]$attempts = 3" in text("scripts/VC-ANDROID-LIVE-CONNECTIVITY-CHECK.ps1"))
