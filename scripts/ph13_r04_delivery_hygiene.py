...
MANIFEST=ROOT/'contracts/VC-ANDROID-PH13-R04-SOURCE-MANIFEST.sha256'
...
if not MANIFEST.is_file(): raise SystemExit('[FAIL] PH13-R04 source manifest missing')
tracked=set()
for line in MANIFEST.read_text(encoding='utf-8').splitlines():
    if not line.strip(): continue
    expected,rel=line.split('  ',1); tracked.add(rel); p=ROOT/rel
    if not p.is_file(): raise SystemExit(f'[FAIL] PH13-R04 delivery file missing: {rel}')
    if hashlib.sha256(p.read_bytes()).hexdigest()!=expected: raise SystemExit(f'[FAIL] PH13-R04 delivery file drifted: {rel}')
...
print(f'[PASS] PH13-R04 delivery manifest verified: {len(tracked)} tracked files')
print('[PASS] PH13-R04 original delivery tree stayed immutable and distributable-clean')
