...
MANIFEST = ROOT / 'contracts/VC-ANDROID-PH13-R16-SOURCE-MANIFEST.sha256'
...
if not MANIFEST.is_file(): raise SystemExit('[FAIL] PH13-R16 source manifest missing')
tracked={}
for line in MANIFEST.read_text(encoding='utf-8').splitlines():
    if not line.strip(): continue
    expected, rel=line.split('  ',1); tracked[rel]=expected
    p=ROOT/rel
    if not p.is_file(): raise SystemExit(f'[FAIL] PH13-R16 delivery file missing: {rel}')
    actual=hashlib.sha256(p.read_bytes()).hexdigest()
    if actual!=expected: raise SystemExit(f'[FAIL] PH13-R16 delivery file drifted: {rel}')
...
print(f'[PASS] PH13-R16 tracked delivery manifest verified: {len(tracked)} files')
...
print('[PASS] PH13-R16 product/delivery integrity is clean')
