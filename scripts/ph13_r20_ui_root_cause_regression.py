...
check('FontFamily.Serif' not in theme,'global VoiceCloud typography no longer uses mismatched serif headings')
check(theme.count('FontFamily.SansSerif') >= 10,'global display/headline/body typography follows clean sans-serif board family')
check((ROOT/'core/designsystem/src/main/res/drawable/vc_nav_live.xml').is_file(),'dedicated microphone navigation vector exists')
check('R.drawable.vc_nav_live' in disc,'consumer center action renders dedicated microphone vector')
check('R.drawable.vc_nav_live' in creator,'creator center action renders dedicated microphone vector')
...
check('Welcome to VoiceCloud' in auth and '.height(250.dp)' in auth,'startup onboarding composition uses controlled phone-sized hero geometry')
check('Speak. Connect.\\nBelong.' not in auth,'rejected oversized multiline startup heading removed')
