...
cd /d "%~dp0.."
echo =============================================================
echo VoiceCloud Android PH06 R03 - Full Acceptance
echo Host/Speaker Room Management, Scheduling, Stage, Polls and Quiz
echo Parent: PH05-R04 85eef801187243453e84bbf7e71af67ba73aef4b
echo Includes: PH06-R02 scope plus HostingScreens nullable-stage compile correction
echo Preservation policy: PH01-PH05 regressions run before Android build gates
echo =============================================================

where powershell >nul 2>nul || (echo [FAIL] PowerShell is required for source-contract verification.& exit /b 1)
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\VC-ANDROID-PH06-R03-SOURCE-CHECK.ps1 || exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\VC-ANDROID-PH06-R02-SOURCE-CHECK.ps1 || exit /b 1
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\VC-ANDROID-PH06-R01-SOURCE-CHECK.ps1 || exit /b 1
...
where python >nul 2>nul && (python scripts\ph06_r03_hosting_compile_regression.py || exit /b 1)
where python >nul 2>nul && (python scripts\ph06_r02_premium_ui_regression.py || exit /b 1)
where python >nul 2>nul && (python scripts\ph06_r01_source_check.py || exit /b 1)
where python >nul 2>nul && (python scripts\ph06_r01_ui_motion_spacing_regression.py || exit /b 1)
where python >nul 2>nul && (python scripts\ph06_r01_module_dependency_audit.py || exit /b 1)
where python >nul 2>nul && (python scripts\ph06_r01_host_runtime_regression.py || exit /b 1)
...
:device_done
echo [PASS] VC-ANDROID-PH06-R03 acceptance commands completed successfully.
