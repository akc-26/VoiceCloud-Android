...
Pass 'real-device debug API resolves through adb reverse loopback' ($resolver.Contains('PhysicalDeviceDebugBaseUrl = "http://127.0.0.1:3000/api/v1/"') -and $resolver.Contains('if (emulator) EmulatorDebugBaseUrl else PhysicalDeviceDebugBaseUrl'))
Pass 'emulator debug API retains 10.0.2.2 host alias' $resolver.Contains('EmulatorDebugBaseUrl = "http://10.0.2.2:3000/api/v1/"')
$foundation = Read-Text 'app/src/main/java/app/voicecloud/android/di/FoundationModule.kt'
Pass 'runtime endpoint resolver feeds Retrofit authority' $foundation.Contains('ApiEndpointResolver.resolve(BuildConfig.API_BASE_URL, BuildConfig.DEBUG)')
...
Write-Host "VC-ANDROID-PH01-R06 source authority: $script:count/$script:count PASS"
