...
$workspace = Join-Path $env:TEMP ("VoiceCloud-PH13-R04-$runId")
$gradleUserHome = Join-Path $env:TEMP ("VoiceCloud-PH13-R04-GRADLE-$runId")
...
function Invoke-GradleGate {
    ...
    $script:total++
    Write-Host $Label
    if (-not (Test-Path -LiteralPath $script:gradlew)) {
        Add-Failure -Label $Label -Detail 'gradlew.bat is unavailable because workspace/bootstrap did not complete.'
        return
    }
    try {
...
try {
    ...
    if (Test-Path -LiteralPath $workspace) {
        ...
        try {
            ...
            if ($bootstrapExit -eq 0 -and (Test-Path -LiteralPath $gradlew)) {
                $script:passes++
                Write-Host '[GATE-PASS] Gradle wrapper bootstrap'
            } else {
    ...
    if (Test-Path -LiteralPath $gradlew) {
        try {
            Write-Host '[CLEANUP] Stopping isolated Gradle daemons...'
