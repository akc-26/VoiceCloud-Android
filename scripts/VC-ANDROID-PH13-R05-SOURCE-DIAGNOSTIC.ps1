...
function Invoke-DiagnosticGate {
    param([Parameter(Mandatory=$true)][string]$Label,[Parameter(Mandatory=$true)][string]$FilePath,[Parameter(Mandatory=$true)][string[]]$Arguments)
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $script:failures.Add("$Label :: exit $LASTEXITCODE")
            Write-Host "[DIAG-FAIL] $Label :: exit $LASTEXITCODE"
        }
    } catch {
...
$pythonChecks = @(
    'ph13_r05_acceptance_wiring_regression.py',
    'ph13_r05_uiux_preservation_regression.py',
...
Write-Host '[SOURCE SUMMARY] All PH13-R05 UI/UX source and functional-preservation diagnostics passed.'
exit 0
