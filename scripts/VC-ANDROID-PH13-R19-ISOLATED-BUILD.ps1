...
$workspace = Join-Path $env:TEMP ("VoiceCloud-PH13-R19-$runId")
...
try {
    ...
    Write-Host "[INFO] Creating isolated R19 full-acceptance workspace: $workspace"
        ...
        if (-not (Test-Path -LiteralPath $deviceScript)) {
            ...
            try {
                ...
                if ($deviceGate -eq 0) {
                    ...
                    Add-Warning -Label '[GATE 7] Physical-device instrumentation' -Detail 'No healthy authorized API 26+ device was available. Host build gates passed, but R19 is NOT fully accepted until this gate runs.'
                } else {
    ...
    Add-Failure -Label 'R19 isolated build harness' -Detail $_.Exception.Message
} finally {
    ...
    $workspaceRemoved = Remove-TreeRobust -Path $workspace -Label 'Isolated R19 Android workspace'
    if (-not $workspaceRemoved) { Add-Warning -Label 'R19 workspace cleanup' -Detail 'Temporary workspace remains outside delivery/Git source tree.' }
...
Write-Host '[PASS] Full R19 Android build + physical-device acceptance gates passed.'
exit 0
