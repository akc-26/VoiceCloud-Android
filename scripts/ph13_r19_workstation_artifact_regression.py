...
with tempfile.TemporaryDirectory(prefix='vc-r19-artifacts-') as td:
 ...
 for script in ['ph13_r19_product_preservation_regression.py','ph13_r19_delivery_integrity.py','ph13_r19_source_hygiene.py']:
 ...
 print(f'[PASS] R19 workstation/generated-artifact simulation accepted: {len(ARTIFACTS)} artifacts including exact Room schema JSON')
 bad=dst/'feature/auth/src/main/java/app/voicecloud/feature/auth/ui/UnexpectedDrift.kt'; bad.write_text('package bad\n',encoding='utf-8')
 failed=0
 for script in ['ph13_r19_product_preservation_regression.py','ph13_r19_delivery_integrity.py']:
  rc=subprocess.run([sys.executable,str(dst/'scripts'/script)],cwd=dst,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,encoding='utf-8',env=env)
  if rc.returncode!=0: failed+=1
 if failed!=2: raise SystemExit('[FAIL] R19 gates did not remain fail-closed for unknown product/source drift')
 print('[PASS] R19 preservation and delivery integrity remain fail-closed for unknown product/source drift')
