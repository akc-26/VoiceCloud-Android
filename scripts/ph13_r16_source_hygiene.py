...
for p in ROOT.rglob('*'):
    ...
    if rel.startswith(ROOM_PREFIX) and rel.endswith('.json'):
        ignored_room.append(rel)
        continue
    if p.suffix.lower() not in TEXT_SUFFIXES and p.name not in {'gradle.properties'}: continue
    try: data=p.read_bytes()
    except OSError as e:
        failures.append(f'unreadable text file {rel}: {e}'); continue
...
if failures:
    ...
    raise SystemExit(1)
print(f'[PASS] R16 source hygiene checked {checked} authored text files')
if ignored_room:
    print(f'[PASS] Ignored {len(ignored_room)} exact Room-generated schema JSON artifact(s); generated formatter output is not authored source')
print('[PASS] R16 authored source has final newlines and no trailing whitespace')
