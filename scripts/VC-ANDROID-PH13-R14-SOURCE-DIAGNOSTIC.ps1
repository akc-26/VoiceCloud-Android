...
function Invoke-DiagnosticGate {
    ...
    try {
        & $FilePath @Arguments
        if ($LASTEXITCODE -eq 0) { $script:passes++; Write-Host "[DIAG-PASS] $Label" }
        else { $script:failures.Add("$Label :: exit $LASTEXITCODE"); Write-Host "[DIAG-FAIL] $Label :: exit $LASTEXITCODE" }
    } catch {
...
$pythonChecks = @(
 'ph13_r14_acceptance_wiring_regression.py',
 'ph13_r14_product_preservation_regression.py',
 'ph13_r14_visual_device_regression.py',
 'ph13_r14_kotlin_compile_risk_regression.py',
 'ph13_r14_windows_harness_structure_regression.py',
 'ph13_r14_source_hygiene.py',
 'ph13_r14_workstation_artifact_regression.py',
 ...
 'ph13_r01_unit_contract_regression.py','ph13_r01_source_check.py','ph13_r01_backend_contract_regression.py','ph13_r01_compile_surface_regression.py',
...
Write-Host '[SOURCE SUMMARY] R14 physical-device-driven UI/UX, text/media, functional preservation and workstation-safe source authority checks passed.'
exit 0
