...
ok("real-device debug API resolves through adb reverse loopback", "PhysicalDeviceDebugBaseUrl = \"http://127.0.0.1:3000/api/v1/\"" in resolver and "if (emulator) EmulatorDebugBaseUrl else PhysicalDeviceDebugBaseUrl" in resolver)
ok("emulator debug API retains 10.0.2.2 host alias", "EmulatorDebugBaseUrl = \"http://10.0.2.2:3000/api/v1/\"" in resolver)
foundation = text("app/src/main/java/app/voicecloud/android/di/FoundationModule.kt")
ok("runtime endpoint resolver feeds Retrofit authority", "ApiEndpointResolver.resolve(BuildConfig.API_BASE_URL, BuildConfig.DEBUG)" in foundation)
...
print(f"VC-ANDROID-PH01-R06 source authority: {len(checks)}/{len(checks)} PASS")
