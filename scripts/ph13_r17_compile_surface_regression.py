...
for p in KT:
    ...
    if rel in seen:
        continue
    seen.add(rel)
    ck(p.is_file(), f'R17 audited Kotlin exists: {rel}')
    if not p.is_file():
        continue
...
# R16/R17 known compiler and warning closure.
...
if fail:
    print(f'[SUMMARY-FAIL] VC-ANDROID-PH13-R17 compile-surface regression: {passes} passed; {len(fail)} failed')
    raise SystemExit(1)
print(f'[PASS] VC-ANDROID-PH13-R17 compile-surface regression: {passes}/{passes} PASS')
