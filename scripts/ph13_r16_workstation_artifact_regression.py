...
with tempfile.TemporaryDirectory(prefix='vc-r16-artifacts-') as td:
 ...
 for script in ['ph13_r16_product_preservation_regression.py','ph13_r16_delivery_integrity.py','ph13_r16_source_hygiene.py']:
  rc=subprocess.run([sys.executable,str(dst/'scripts'/script)],cwd=dst,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
  if rc.returncode:
   print(rc.stdout); raise SystemExit(f'[FAIL] generated-artifact simulation failed in {script}')
 print(f'[PASS] R16 workstation/generated-artifact simulation accepted: {len(ARTIFACTS)} artifacts including exact Room schema JSON')
 bad=dst/'feature/auth/src/main/java/app/voicecloud/feature/auth/ui/UnexpectedDrift.kt'; bad.write_text('package bad\n')
 failed=0
 for script in ['ph13_r16_product_preservation_regression.py','ph13_r16_delivery_integrity.py']:
  rc=subprocess.run([sys.executable,str(dst/'scripts'/script)],cwd=dst,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
  if rc.returncode!=0: failed+=1
 if failed!=2: raise SystemExit('[FAIL] R16 gates did not remain fail-closed for unknown product/source drift')
 print('[PASS] R16 preservation and delivery integrity remain fail-closed for unknown product/source drift')
