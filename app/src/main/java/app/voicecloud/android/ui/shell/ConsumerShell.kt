...
import app.voicecloud.android.ui.consumer.ConsumerDiscoveryUiState
...
fun ConsumerShell(
    ...
    val discovery = remember { ConsumerDiscoveryUiState() }
    ...
    val sides = remember {
        listOf(
            ...
            VCNavDestination(ConsumerDestinations.Discover, "Explore", VoiceCloudIcons.Explore, VoiceCloudIcons.ExploreSelected),
    ...
    VCScaffold(
        ...
        NavHost(
            ...
            composable(ConsumerDestinations.Home) {
                HomeScreen(state = discovery, onOpenSearch = openSearch)
            }
            composable(ConsumerDestinations.Discover) {
                ExploreScreen(state = discovery, onOpenSearch = openSearch)
            }
            composable(ConsumerDestinations.Search) {
                SearchScreen(
                    state = searchState,
                    onQueryChange = { query -> searchState = searchState.copy(query = query, errorMessage = null) },
                    onSearch = { searchState = searchState.copy(isSearching = searchState.query.isNotBlank(), isLoading = false) },
